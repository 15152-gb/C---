    ...
    while (flag == 1 || curr != interCodes) {
        ...
        switch (curr->kind) {
            ...
            case FUNC_IR: {
                // 函数名 curr->ops[0]->name
                fprintf(fp, "\n%s:\n", curr->ops[0]->name);
                // 栈帧初始化：将 $fp（帧指针）压栈，并将 $sp（栈指针）的值赋给 $fp，从而标志着新的栈帧的开始（从$fp开始）
                ...
                if (strcmp(curr->ops[0]->name, "main") != 0) {
                    // 将所有可用寄存器的值压栈，保存当前函数调用时的寄存器状态，防止在函数调用时寄存器内容丢失
                    pushAllRegs(fp);
                    // 为局部变量分配空间
                    fprintf(fp, "  addi $sp, $sp, %d\n", -frame->vars->offset + 72);
                }
                else
                    // 在栈上为局部变量分配空间，栈指针只移动 frame->vars->offset 字节。
                    fprintf(fp, "  addi $sp, $sp, %d\n", -frame->vars->offset);
                // 清除寄存器的状态，准备处理函数参数
                clearRegs();
                // 处理函数的参数声明（即FUNC_IR指令后的PARAM_IR指令）
                int argCount = 0;
            ...
            case ASSIGN_IR: {
                ...
                // - 如果是取值操作（例如取指针指向的值），则使用 sw 指令将右操作数存储到左操作数指向的内存地址
                    ...
                    fprintf(fp, "  sw %s, 0(%s)\n", regs[regRight]->name, regs[regLeft2]->name);
                }
                break;
            }
            case PLUS_IR: {
                ...
                // 处理右操作数：对每个右操作数（right1 和 right2），通过 handleOp(right, fp, 1) 函数将其转换为对应的寄存器，并获取其在寄存器中的索引，即将右操作数被正确加载到寄存器中，以供后续运算使用
                int regRight1 = handleOp(right1, fp, 1);
                int regRight2 = handleOp(right2, fp, 1);
                // 处理左操作数：分类处理，然后运算并存储结果
                // 变量或临时变量
                if (left->kind == VARIABLE_OP || left->kind == TEMP_VAR_OP) {
                    int regLeft = getReg(left, fp, 0);
            ...
            case SUB_IR: {
                // TODO1：实现SUB_IR的目标代码生成，可仿照PLUS_IR的过程
                // 取操作数
                Operand left = curr->ops[0];
                Operand right1 = curr->ops[1];
                Operand right2 = curr->ops[2];
            ...
            case GOTO_IR: {
                // 无条件跳转：通过 j label%d 指令实现跳转，标签编号由 curr->ops[0]->no 提供
                fprintf(fp, "  j label%d\n", curr->ops[0]->no);
                break;
            }
            case IF_GOTO_IR: {
                // 有条件跳转，根据给定的条件（两个操作数的比较）生成条件跳转指令
                Operand left = curr->ops[0];
                Operand right = curr->ops[1];
            ...
            case RETURN_IR: {
                ...
                // 如果不是main函数，那么要恢复栈帧，被调用函数需要将所有可操作寄存器保存到栈中并清空可操作寄存器
            ...
            case CALL_IR: {
                ...
                int argCount = 0;
                // 为前四个之后的参数分配的寄存器编号
                // 如果存在一个参数特别多的函数的话，这个数组的大小可能需要调整，
                int regNos[64];
            ...
            case PARAM_IR:
                ...
                break;
            case READ_IR: {
                // 将栈指针（$sp）减少 4，创建栈空间，为接下来的栈操作（保存 $ra 寄存器）腾出空间
                fputs("  addi $sp, $sp, -4\n", fp);
                // 将返回地址寄存器 $ra 保存到栈上（栈指针 $sp 指向的位置）。这样做是为了在函数调用之后能够恢复返回地址，确保程序的控制流正确
		        fputs("  sw $ra, 0($sp)\n", fp);
                // 调用 read 函数
		        fputs("  jal read\n", fp);
                // 从栈顶加载 $ra 寄存器的值，恢复返回地址。这样read函数执行完后，程序能正确跳回到调用函数的地方。
		        fputs("  lw $ra, 0($sp)\n", fp);
                // 恢复栈指针，将栈指针回到原来位置
		        fputs("  addi $sp, $sp, 4\n", fp);
                // 存储 read 函数的返回值
                // 读变量（VARIABLE_OP）或临时变量（TEMP_VAR_OP）
		        if (curr->ops[0]->kind == VARIABLE_OP || curr->ops[0]->kind == TEMP_VAR_OP) {
                    int regNo = getReg(curr->ops[0], fp, 0);
                    // 将 read 函数的返回值（存储在 $v0 中）移动到为目标操作数分配的寄存器中
		            fprintf(fp, "  move %s, $v0\n", regs[regNo]->name);
                    spillReg(regs[regNo], fp);
                }
                // 对一个内存地址的操作，目标是将读取的值存储到一个内存位置
                else if (curr->ops[0]->kind == GET_VAL_OP) {
                    int regNo = getReg(curr->ops[0]->opr, fp, 1);
                    // 将 $v0 寄存器的值（即读取的输入值）存储到目标地址（由寄存器 regs[regNo]->name 指向的内存位置）
                    fprintf(fp, "  sw $v0, 0(%s)\n", regs[regNo]->name);
                }
                break;
            }
            case WRITE_IR: {
                // TODO2：实现WRITE_IR的目标代码生成，已给出具体提示，可参考READ_IR的过程
                // 调用 handleOp 函数，处理 WRITE_IR 的操作数 curr->ops[0]，并为其分配一个寄存器
                int regNo = handleOp(curr->ops[0], fp, 1);
                // 将需要输出的值从 上一步分配的寄存器 移动到 $a0 寄存器 中
                fprintf(fp, "  move $a0, %s\n", regs[regNo]->name);
                // READ_IR那五条fputs的逆过程
                // 将栈指针（$sp）减少 4，创建栈空间，为保存 $ra 寄存器腾出空间
                fputs("  addi $sp, $sp, -4\n", fp);
                // 将返回地址寄存器 $ra 保存到栈上
                fputs("  sw $ra, 0($sp)\n", fp);
                // 调用 write 函数
                fputs("  jal write\n", fp);
                // 从栈顶加载 $ra 寄存器的值，恢复返回地址
                fputs("  lw $ra, 0($sp)\n", fp);
                // 恢复栈指针，将栈指针回到原来位置
                fputs("  addi $sp, $sp, 4\n", fp);
