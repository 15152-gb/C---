...
void check() {
    for (int i = 0; i < HASH_SIZE; i++) {
        Entry cur = symbolTable[i];
        while (cur != NULL) {
            if (cur->type->kind == ENUM_FUNC && cur->type->func->hasDefined == 0) {
                printf("Error type 18 at line %d: Undefined function \"%s\".\n", cur->type->func->lineno, cur->name);
            }
            cur = cur->hashNext; // 移动到链表的下一个节点
        }
    }
}
...
void ExtDef(Node* root) {
    ...
    if (strcmp(root->children[1]->name, "FunDec") == 0) {
        ...
        if (sym != NULL) {
            ...
            // 符号表中存在与当前函数/声明同名的函数声明, 识别错误信息type 19，(1)当前函数定义和同名声明冲突，（2）当前声明和同名声明冲突
            ...
            if (sym->type->func->hasDefined == 0) {
                ...
                // 当前声明与同名声明一致，为已声明的函数添加定义
                if (strcmp(root->children[2]->name, "CompSt") == 0) {
                    sym->type->func->hasDefined = 1; // 更新为已定义
...
Type Exp(Node* root) {
    //printTree(root->children[0],0); // 打印左侧节点
    if (strcmp(root->children[0]->name, "Exp") == 0) {
        ...
        // 数组取地址操作
            ...
            // 2、域
            // 3、数组元素
            // 如果左值合法，返回左值的类型Exp(left)，反之返回NULL
            // 在这段代码中实现对type5 和 type6的检查
            // "Error type 6 at line %d: The left-hand side of an assignment must be a variable.\n"
            // "Error type 5 at line %d: Type mismatched for assignment.\n"
            if (root->childNum < 3) {
                fprintf(stderr, "Error: Incomplete ASSIGNOP expression at line %d.\n", root->lineno);
                return NULL;
            }

            Type left = Exp(root->children[0]);  // 左侧表达式类型
            Type right = Exp(root->children[2]); // 右侧表达式类型
            ...
            if (leftNode == NULL || leftNode->childNum < 1) {
                fprintf(stderr, "Error: Invalid left-hand expression at line %d.\n", root->lineno);
                return NULL;
            }

            if (strcmp(leftNode->name, "INT") == 0 || strcmp(leftNode->name, "FLOAT") == 0) {
            ...
            // 检查左值是否合法
            int isLeftValue = 0;
            if (leftNode->childNum >= 1 && strcmp(leftNode->children[0]->name, "ID") == 0) {
                isLeftValue = 1;
            }
            if (leftNode->childNum >= 2 && 
                (strcmp(leftNode->children[1]->name, "DOT") == 0 || strcmp(leftNode->children[1]->name, "LB") == 0)) {
                isLeftValue = 1;
            }
